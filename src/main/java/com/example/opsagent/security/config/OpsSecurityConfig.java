...
public class OpsSecurityConfig {
    ...
    private static final String[] ADMIN_API_PATTERNS = {
        "/api/audit/**", "/api/notifications/**", "/api/tasks/**"
    };
    ...
    @Bean
    public OpsAuthenticationManager opsAuthenticationManager(
            OpsUserDetailsService userDetailsService, PasswordEncoder passwordEncoder) {
    ...
    public SecurityFilterChain securityFilterChain(
            HttpSecurity http,
            OpsAuthenticationManager authenticationManager,
            ObjectMapper objectMapper,
            OpsAuthenticationSuccessHandler successHandler,
            OpsAuthenticationFailureHandler failureHandler,
            OpsTokenService tokenService,
            OpsUserDetailsService userDetailsService,
            OpsAuthenticationEntryPoint authenticationEntryPoint,
            OpsAccessDeniedHandler accessDeniedHandler,
            OpsSecurityProperties securityProperties)
            throws Exception {
        OpsLoginAuthenticationFilter loginFilter =
                new OpsLoginAuthenticationFilter(authenticationManager, objectMapper);
        loginFilter.setAuthenticationSuccessHandler(successHandler);
        loginFilter.setAuthenticationFailureHandler(failureHandler);

        OpsJwtAuthenticationFilter jwtFilter =
                new OpsJwtAuthenticationFilter(
                        tokenService,
                        userDetailsService,
                                ...
                                authorize
                                        .requestMatchers(
                                                securityProperties
                                                        ...
                                                        .toArray(String[]::new))
                                        .permitAll()
                                        .requestMatchers(ADMIN_API_PATTERNS)
                                        .hasRole("ADMIN")
                                        .anyRequest()
                                        .authenticated())
