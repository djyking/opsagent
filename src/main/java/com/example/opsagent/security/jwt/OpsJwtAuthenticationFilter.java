...
public class OpsJwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    public OpsJwtAuthenticationFilter(OpsTokenService tokenService, OpsUserDetailsService userDetailsService,
        OpsAuthenticationEntryPoint authenticationEntryPoint, OpsSecurityProperties securityProperties) {
        this.tokenService = tokenService;
        this.userDetailsService = userDetailsService;
        this.authenticationEntryPoint = authenticationEntryPoint;
        List<RequestMatcher> matchers = securityProperties.getPermitAll().stream()
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            OpsUserPrincipal principal = (OpsUserPrincipal) userDetailsService.loadUserByUsername(
                tokenService.extractUsername(claims));
            if (!principal.isEnabled() || !principal.getUserId().equals(tokenService.extractUserId(claims))) {
            ...
            ((OpsAuthenticationToken) authentication).setDetails(
            ...
            authenticationEntryPoint.commence(request, response,
                new BadCredentialsException("JWT 认证失败", exception));
            return;
        }
        filterChain.doFilter(request, response);
    }
