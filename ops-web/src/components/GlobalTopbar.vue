...
import { Bot, ChevronDown, FileUp, Menu, Plus, Search, Settings2, ShieldCheck, TicketCheck } from "@lucide/vue";
import { ref, watch } from "vue";
import { useRoute, useRouter } from "vue-router";
import { managementNavigation } from '@/data/navigation';
...
async function newAssistant() { await assistant.newSession(); if (createMenu.value) createMenu.value.open = false; assistant.show(); }
</script>
...
<template>
  <header class="global-topbar">
    ...
    <details ref="createMenu" class="topbar-create-menu"><summary class="button primary topbar-create"><Plus :size="16" :stroke-width="1.75" />创建<ChevronDown :size="16" :stroke-width="1.75" /></summary><div class="topbar-create-popover"><RouterLink v-if="!auth.isDemo" to="/tickets?create=1"><TicketCheck :size="16" /><span><strong>新建事件</strong><small>记录影响、分派责任并跟进处置</small></span></RouterLink><button type="button" class="topbar-new-assistant" :disabled="assistant.busy" @click="newAssistant"><Bot :size="16" /><span><strong>助手新会话</strong><small>通用问答与知识检索</small></span></button><RouterLink v-if="!auth.isDemo" to="/knowledge"><FileUp :size="16" /><span><strong>上传知识文档</strong><small>进入知识库并选择目标库</small></span></RouterLink></div></details>
    <button type="button" class="button secondary topbar-assistant" aria-label="AI 助手，分析当前页面上下文" aria-haspopup="dialog" :aria-expanded="assistant.open" title="AI 助手 · 分析当前页面上下文" @click="assistant.show()"><Bot :size="17" /><span>AI 助手</span></button>
...
<style scoped>
.topbar-new-assistant { width: 100%; display: flex; gap: 10px; padding: 10px; border: 0; border-radius: var(--oa-radius-control); background: transparent; text-align: left; color: inherit; cursor: pointer; font: inherit; }
.topbar-new-assistant:hover { background: var(--oa-bg-subtle); }
.topbar-new-assistant:disabled { opacity: .55; cursor: not-allowed; }
