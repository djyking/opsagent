...
<template>
  <BaseModal v-if="inbox.open" title="待处理审批" description="核对本次动作后决定。选择稍后处理，可以从任意页面右上角再次打开。" wide @close="inbox.later">
    <div class="approval-inbox">
      ...
      <nav v-if="inbox.availableItems.length > 1 || (!inbox.selected && inbox.count)" class="approval-inbox-list" aria-label="选择待审批动作"><button v-for="item in inbox.availableItems" :key="approvalKey(item)" type="button" :disabled="!!inbox.busy" :aria-pressed="inbox.selected?.id === item.id" @click="decisionError = ''; inbox.select(item)"><ShieldCheck :size="17" /><span><strong>{{ approvalDescription(item).title }}</strong><small>{{ item.ticketId > 0 ? `工单 #${item.ticketId}` : approvalDescription(item).configuration ? '配置变更' : '独立工作流' }} · {{ item.nodeLabel }}</small></span></button></nav>
