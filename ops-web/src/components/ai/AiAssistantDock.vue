...
<template>
  <section v-if="visible" ref="dock" class="ai-assistant-dock" role="dialog" aria-modal="false" aria-labelledby="ai-dock-title" @keydown.esc.stop="close">
    ...
    <div class="ai-dock-context"><span>当前上下文</span><strong>{{ assistant.contextSummary || '全局运维问答' }}</strong><small>只提交服务与窗口引用；后端重新鉴权并读取证据。</small><div class="row-actions"><button v-if="assistant.contextSummary" type="button" class="text-button" :disabled="assistant.busy" @click="assistant.setContext({})">移除上下文</button><RouterLink class="text-button" to="/observability/topology" @click="close">选择其他服务</RouterLink></div></div>
