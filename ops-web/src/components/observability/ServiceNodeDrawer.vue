...
const historyLoading = ref(false); const configLoading = ref(false); const trafficLoading = ref(false);
const configError = ref(''); const trafficError = ref('');
let epoch = 0; let timer: ReturnType<typeof setInterval> | undefined; let disposed = false;
type Section = 'detail' | 'history' | 'config' | 'traffic';
const requests = new Map<Section, AbortController>();
...
async function loadSection(section: Section) {
  if (disposed || !auth.identity || requests.has(section)) return;
  const own = epoch, code = props.node.ciCode, identity = auth.identity;
  const controller = new AbortController(); requests.set(section, controller);
  const pending = { detail: loading, history: historyLoading, config: configLoading, traffic: trafficLoading }[section];
  const failure = { detail: error, history: historyError, config: configError, traffic: trafficError }[section];
  pending.value = true; failure.value = '';
  const options = { signal: controller.signal, expectedIdentity: identity };
  const filters = { environment: props.environment, timeRange: props.timeRange };
  const valid = () => !disposed && !controller.signal.aborted && own === epoch && identity === auth.identity;
  try {
    if (section === 'detail') {
      const result = await observabilityApi.service(code, filters, options);
      if (!valid()) return;
      if (result.node.ciCode !== code) throw new Error('服务详情与当前选择不匹配');
      detail.value = result;
    } else if (section === 'history') {
      const result = await observabilityApi.metricHistory(code, filters, options);
      if (!valid()) return;
      if (result.ciCode && result.ciCode !== code) throw new Error('历史样本与当前服务不匹配');
      history.value = result;
    } else if (section === 'config') {
      const result = await observabilityApi.configSummary(code, options);
      if (valid()) configSummary.value = result;
    } else {
      const result = await observabilityApi.trafficSummary(code, options);
      if (valid()) trafficSummary.value = result;
    }
  } catch (cause) {
    if (valid()) {
      const message = cause instanceof Error ? cause.message : '请求失败';
      const labels = { detail: '服务详情', history: '历史趋势', config: '配置摘要', traffic: '流量摘要' };
      failure.value = `${labels[section]}暂未更新：${/timeout|超时/i.test(message) ? '读取超时，请重试' : message}`;
    }
  } finally {
    if (requests.get(section) === controller) { requests.delete(section); pending.value = false; }
  }
}
function load() { return loadSection('detail'); }
function loadHistory() { return loadSection('history'); }
function refreshVisible() {
  void load();
  if (tab.value === 'overview' || tab.value === 'metrics') void loadHistory();
  if (tab.value === 'config' || tab.value === 'traffic') void loadSection(tab.value);
}
function clearRequests() {
  epoch++; requests.forEach(controller => controller.abort()); requests.clear();
  loading.value = historyLoading.value = configLoading.value = trafficLoading.value = false;
}
...
function analyze() { emit('close'); ai.show({ service: props.node.ciCode, environment: props.environment, timeRange: props.timeRange }); ai.question = '请分析当前服务的异常现象、证据和可能原因，并说明还需要核对哪些信息。'; }
...
watch([() => props.node.ciCode, () => props.environment, () => props.timeRange, () => auth.identity], () => {
  clearRequests(); detail.value = undefined; configSummary.value = undefined; trafficSummary.value = undefined; history.value = undefined;
  error.value = historyError.value = configError.value = trafficError.value = ''; tab.value = 'overview';
  if (auth.identity) refreshVisible();
});
watch(tab, value => { if (value === 'config' || value === 'traffic') void loadSection(value); });
onMounted(() => { refreshVisible(); timer = setInterval(() => { if (!document.hidden) refreshVisible(); }, 15_000); });
onBeforeUnmount(() => { disposed = true; clearRequests(); clearInterval(timer); });
</script>
<template>
  <ServiceDetailShell :title="node.ciName" :subtitle="node.ciCode" :inline="inline" @close="emit('close')">
    ...
    <div v-if="error" class="obs-section-feedback"><InlineError :message="error" /><button class="text-button" :disabled="loading" @click="load">重试详情</button></div>
    <p class="obs-detail-refresh" aria-live="polite">{{ loading ? (detail ? '正在更新服务详情…' : '正在读取服务详情…') : detail ? `详情更新于 ${observationTime(detail.checkedAt || detail.node.observedAt)}` : '先展示拓扑已有样本，详情尚未取得' }}<span v-if="error && detail"> · 保留上次取得的数据</span></p>
    ...
    <div v-else-if="tab === 'traffic'" class="obs-drawer-section"><LoadingState v-if="trafficLoading && !trafficSummary" text="正在读取流量摘要…" /><div v-if="trafficError" class="obs-section-feedback"><InlineError :message="trafficError" /><button class="text-button" :disabled="trafficLoading" @click="loadSection('traffic')">重试流量摘要</button></div><small v-if="trafficSummary?.observedAt" class="obs-history-caption">更新于 {{ observationTime(trafficSummary.observedAt) }}</small><div class="obs-metric-grid"><article><span>Sentinel 通过 QPS</span><strong>{{ metric(trafficSummary?.passQps, ' /s') }}</strong></article><article><span>Sentinel 阻断</span><strong>{{ metric(trafficSummary?.blockQps, ' /s') }}</strong></article><article><span>已观测资源</span><strong>{{ trafficSummary?.resourceCount ?? '—' }}</strong></article><article><span>生效规则</span><strong>{{ trafficSummary?.ruleCount ?? '—' }}</strong></article></div><p class="obs-muted">{{ trafficSummary?.message }} 规则由流量治理统一管理。当前绑定：{{ current.bindings?.sentinelApp || '未登记 Sentinel App' }}</p><button class="button primary" @click="go('/observability/traffic')"><Gauge :size="16" />查看流量与规则</button></div>
    <div v-else-if="tab === 'config'" class="obs-drawer-section"><LoadingState v-if="configLoading && !configSummary" text="正在读取配置摘要…" /><div v-if="configError" class="obs-section-feedback"><InlineError :message="configError" /><button class="text-button" :disabled="configLoading" @click="loadSection('config')">重试配置摘要</button></div><small v-if="configSummary?.observedAt" class="obs-history-caption">更新于 {{ observationTime(configSummary.observedAt) }}</small><div class="obs-metric-grid"><article><span>配置项</span><strong>{{ configSummary?.configurationCount ?? '—' }}</strong></article><article><span>受控可编辑项</span><strong>{{ configSummary?.editableCount ?? '—' }}</strong></article></div><div class="obs-source-note"><span v-for="source in configSummary?.sources" :key="source.source">{{ source.source }} · {{ source.count }} 项</span><span>{{ configSummary?.message }}</span></div><p class="obs-muted">配置绑定：{{ current.bindings?.nacosDataId || '未登记 Nacos Data ID' }}</p><div v-for="change in detail?.recentChanges" :key="String(change.id || change.revision)" class="obs-record"><strong>{{ title(change) }}</strong><small>{{ observationTime(String(change.occurredAt || change.createdAt || change.changedAt || '')) }} · {{ change.actor }} · {{ change.revision }}</small></div><EmptyState v-if="!detail?.recentChanges.length" title="暂无可见变更记录" description="在配置中心核对来源、版本和发布记录。" /><button class="button primary" @click="go('/observability/config')"><FileCog :size="16" />进入配置中心</button></div>
...
<style scoped>
.obs-detail-refresh { margin: 10px 0; color: var(--oa-text-secondary); font-size: 12px; line-height: 1.6; }
.obs-section-feedback { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; }
</style>
