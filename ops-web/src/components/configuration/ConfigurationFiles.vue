...
import ConfigurationValue from '@/components/configuration/ConfigurationValue.vue';
...
const dirty = computed(() => canEdit.value && !!detail.value && (rawMode.value ? raw.value !== detail.value.redactedContent : detail.value.fields.some(field => field.sensitive ? replaceSecrets[field.key] : JSON.stringify(values[field.key]) !== JSON.stringify(field.type === 'json' ? JSON.stringify(field.value, null, 2) : field.value))));
...
function shown(value: unknown) { return value == null ? '未设置' : typeof value === 'object' ? JSON.stringify(value, null, 2) : String(value); }
function jsonValue(value: unknown) { try { return typeof value === 'string' ? JSON.parse(value) : value; } catch { return value; } }
...
async function select(fileId: string, force = false) {
  ...
  try { const value = await api.detail(fileId); if (current !== epoch || disposed) return; detail.value = value; resetForm(value); if (value.lastTaskId && !auth.isDemo) await readTask(value.lastTaskId, current); }
...
<template>
  <section class="managed-files" aria-label="真实配置文件管理">
    <p v-if="!canRead" class="panel managed-file-empty">当前账号可查看运行状态；真实配置文件需管理员、运维或访客只读身份读取。</p><InlineError v-if="error" :message="error" /><div v-if="canRead" class="managed-file-layout">
      <aside class="panel managed-file-directory"><header><strong>服务配置文件</strong><button class="icon-button" :disabled="!!busy" aria-label="刷新配置文件目录" @click="load"><RefreshCw :size="16" /></button></header><label class="search-box"><Search :size="15" /><input v-model="keyword" placeholder="搜索服务或文件" aria-label="搜索受控文件" /></label><nav class="managed-file-list" aria-label="服务配置文件目录"><button v-for="file in filtered" :key="file.id" class="managed-file-item" :class="{ active: file.id === selectedId }" :aria-current="file.id === selectedId ? 'true' : undefined" :disabled="!!busy" @click="select(file.id)"><FileCog :size="19" /><span><strong>{{ file.label }}</strong><small>{{ file.serviceId }} · {{ file.format.toUpperCase() }}</small></span></button></nav><p v-if="!filtered.length" class="managed-file-note">{{ busy ? '正在读取目录…' : files.length ? '没有匹配的配置文件。' : '尚未取得受控文件，请刷新或核对执行器接入。' }}</p></aside>
      <main class="panel managed-file-content">
        <template v-if="detail"><header class="managed-file-heading"><div><h2>{{ detail.label }}</h2><p>{{ detail.serviceId }} · {{ detail.format.toUpperCase() }} · {{ detail.fields.length }} 项纳管参数</p></div><span class="managed-version">文件版本 {{ detail.version.slice(0, 10) }}</span></header>
          ...
          <div v-if="!rawMode" class="managed-fields">
            <form :key="`${selectedId}:${category}`" class="managed-parameter-form" @submit.prevent="createDraft('PUBLISH_ONLY')">
              <div v-for="field in visibleFields" :key="field.key" class="managed-field">
                <div class="managed-field-label"><strong>{{ field.label }}</strong><small v-if="field.unit">{{ field.unit }}{{ field.min != null ? ` · 最小 ${field.min}` : '' }}{{ field.max != null ? ` · 最大 ${field.max}` : '' }}</small><details><summary>参数名称</summary><code>{{ field.key }}</code></details></div>
                <div class="managed-field-input">
                  <ConfigurationValue v-if="!canEdit || !field.editable" :value="field.value" :type="field.type" :label="field.label" :sensitive="field.sensitive" :has-value="field.hasValue" />
                  <template v-else-if="field.sensitive"><label class="managed-secret-toggle"><input v-model="replaceSecrets[field.key]" type="checkbox" :disabled="taskRunning" /><span>{{ field.hasValue ? '已设置 · 替换新值' : '未设置 · 设置新值' }}</span></label><input v-if="replaceSecrets[field.key]" v-model="values[field.key]" type="password" autocomplete="new-password" :aria-label="field.label" :disabled="!!busy || taskRunning" placeholder="仅提交新值，不回显原值" /></template>
                  <select v-else-if="field.type === 'boolean'" v-model="values[field.key]" :aria-label="field.label" :disabled="!!busy || taskRunning"><option :value="true">开启</option><option :value="false">关闭</option></select>
                  <select v-else-if="field.options?.length" v-model="values[field.key]" :aria-label="field.label" :disabled="!!busy || taskRunning"><option v-for="option in field.options" :key="String(option)" :value="String(option)">{{ option }}</option></select>
                  <ConfigurationValue v-else-if="field.type === 'json'" :value="jsonValue(values[field.key])" type="json" :label="field.label"><template #editor><textarea v-model="values[field.key] as string" class="managed-json-input" rows="6" :aria-label="field.label" :disabled="!!busy || taskRunning" spellcheck="false" /></template></ConfigurationValue>
                  <input v-else v-model="values[field.key]" :type="['integer','number'].includes(field.type) ? 'number' : 'text'" :step="field.type === 'integer' ? 1 : 'any'" :min="field.min" :max="field.max" :aria-label="field.label" :disabled="!!busy || taskRunning" autocomplete="off" />
                </div>
              </div>
              <p v-if="!visibleFields.length" class="managed-file-note">此分类暂无已接入的可管理参数。</p>
            </form>
          </div>
          ...
          <details class="managed-file-fold"><summary>{{ auth.isDemo ? '纳管参数与文件版本' : '原文件、加载证据与执行选项' }}</summary><div><p>{{ auth.isDemo ? '以下为真实文件中已纳管参数的脱敏视图；敏感值只显示设置状态，未纳管字段及原文件注释不公开。' : '文件发布与运行采用分别记录；仅发布不主动重启或热加载。' }}</p><dl><div><dt>文件版本</dt><dd>{{ detail.version }}</dd></div><div v-if="!auth.isDemo"><dt>已加载版本</dt><dd>{{ detail.loadedVersion || '尚未取得版本证据' }}</dd></div><div><dt>影响服务</dt><dd>{{ detail.affectedServices.join('、') }}</dd></div></dl><pre>{{ detail.redactedContent }}</pre><button v-if="canEdit && detail.rawEditable && !detail.sensitive" class="button secondary" :disabled="dirty && !rawMode" @click="toggleRawMode">{{ rawMode ? '返回参数表单' : '编辑原文件' }}</button><label v-if="canEdit" class="managed-secret-toggle"><input v-model="rollback" type="checkbox" />将执行失败后的受控恢复纳入本次审批</label></div></details>
          ...
          <details class="managed-file-fold" @toggle="($event.target as HTMLDetailsElement).open && readHistory()"><summary>{{ auth.isDemo ? '已发布版本与脱敏差异' : '草稿、审批与版本记录' }}</summary><div><template v-if="auth.isDemo"><p v-if="history && !history.versions?.length">暂无已发布版本。</p><article v-for="version in history?.versions || []" :key="version.id" class="managed-public-version"><header><strong>{{ state(version.status) }}</strong><time>{{ date(version.createdAt) }}</time></header><p>版本 {{ version.targetVersion.slice(0, 12) }}</p><details><summary>查看脱敏差异</summary><table class="managed-diff"><thead><tr><th>参数</th><th>变更前</th><th>变更后</th></tr></thead><tbody><tr v-for="change in version.diff" :key="change.key"><td>{{ change.label }}</td><td>{{ shown(change.before) }}</td><td>{{ shown(change.after) }}</td></tr></tbody></table></details></article></template><template v-else><p v-if="history && !history.drafts.length">暂无变更记录。</p><article v-for="item in history?.drafts || []" :key="item.id" class="managed-history-row"><div><strong>{{ state(item.status) }} · {{ approvedAction(item.action) }}</strong><small>{{ date(item.createdAt) }} · 用户 #{{ item.createdBy }}</small></div><button class="button secondary small" @click="openDraft(item)">核对差异与审批</button><button v-if="item.taskId" class="text-button" @click="readTask(item.taskId)">执行结果</button></article></template></div></details>
