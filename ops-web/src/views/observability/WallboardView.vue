...
const nodes = computed(() => { void store.observationClock; return [...(store.topologyData?.nodes || [])]; });
const healthy = computed(() => nodes.value.filter(node => effectiveHealth(node, store.observationClock) === 'HEALTHY').length);
const attention = computed(() => nodes.value.filter(node => ['CRITICAL', 'DEGRADED', 'DRILLING'].includes(effectiveHealth(node, store.observationClock))));
const unknown = computed(() => nodes.value.filter(node => effectiveHealth(node, store.observationClock) === 'UNKNOWN').length);
