...
import { effectiveHealth, observationLabels, observationReason, observationState, observationTime, serviceContext } from '@/utils/observability';
...
const filtered = computed(() => [...(store.topologyData?.nodes || [])].filter(node => `${node.ciName} ${node.ciCode} ${node.ownerName || ''} ${node.ciType}`.toLowerCase().includes(search.value.trim().toLowerCase())).sort((a, b) => Number(!/rabbitmq/i.test(`${a.ciCode} ${a.ciName}`)) - Number(!/rabbitmq/i.test(`${b.ciCode} ${b.ciName}`)) || a.ciName.localeCompare(b.ciName, 'zh-CN')));
