...
<template>
  <div class="stack-page knowledge-page">
    <PageHeader :icon="BookOpen" title="知识与经验" description="把团队文档与处置经验，整理成可以查找与复用的知识">
      <template #actions><button v-if="!auth.isDemo" class="button secondary" @click="createOpen = true"><Plus :size="16" />新建知识库</button></template>
    </PageHeader>
    ...
    <main class="panel knowledge-main">
      ...
      <div v-if="selectedBase && !auth.isDemo" class="knowledge-upload" :class="{ 'drag-active': dragActive }" @dragenter.prevent="dragActive = true" @dragover.prevent="dragActive = true" @dragleave.prevent="dragActive = false" @drop.prevent="chooseFile($event.dataTransfer?.files?.[0])">
