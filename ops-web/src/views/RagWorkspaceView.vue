...
<template>
  <div class="stack-page rag-page">
    ...
    <p v-if="assistant.contextSummary" class="rag-shared-context">当前分析上下文：{{ assistant.contextSummary }} <button type="button" class="button text" :disabled="assistant.busy" @click="assistant.setContext({ page: '/rag/chat' })">清除上下文</button></p>
      ...
      <main class="ai-panel rag-workspace">
        <header class="panel-header">
          ...
          <div class="rag-header-actions">
            ...
            <button class="icon-button rag-context-toggle" :aria-expanded="contextOpen" :aria-label="contextOpen ? '收起参考上下文' : '展开参考上下文'" @click="contextOpen = !contextOpen"><PanelRightClose v-if="contextOpen" :size="17" /><PanelRightOpen v-else :size="17" /></button>
          </div>
        </header>
