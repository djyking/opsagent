...
import { ciType, relationNames, environmentNames } from "@/components/cmdb/topology";
...
const environmentLabels = environmentNames;
...
async function load() {
  ...
  error.value = "";
  try {
    cis.value = (await itsmApi.cis()).sort((left, right) => {
      const rank = (ci: Record<string, unknown>) => /rabbitmq/i.test(`${ci.ciCode} ${ci.ciName}`) ? 0 : 1;
...
<template>
  <div class="stack-page cmdb-page">
    <PageHeader v-if="!embedded" :icon="Network" title="服务目录与依赖拓扑" description="找到服务，了解关联依赖与当前配置上下文"><template #actions><button class="button secondary" :disabled="loading || topologyLoading" @click="load"><RefreshCw :size="16" :class="{ 'motion-spin': loading }" />刷新目录</button><button v-if="auth.isAdmin" class="button primary" @click="openCi()"><Plus :size="16" />新增配置项</button></template></PageHeader>
    <div v-else class="row-actions cmdb-embedded-actions"><span>配置台账与实时健康分别核对</span><button class="button secondary" :disabled="loading || topologyLoading" @click="load"><RefreshCw :size="16" />刷新目录</button><button v-if="auth.isAdmin" class="button primary" @click="openCi()"><Plus :size="16" />新增配置项</button></div>
    ...
    <section class="itsm-split">
      ...
      <aside v-if="selectedCi" class="panel cmdb-inspector"><header class="panel-header"><div><h3>服务摘要</h3><p>当前选中配置项</p></div><span class="cmdb-summary-icon"><component :is="ciType(selectedCi.ciType).icon" :size="20" /></span></header><div class="cmdb-summary-identity"><strong>{{ selectedCi.ciName }}</strong><StatusBadge :value="String(selectedCi.status)" /></div><dl class="oa-definition-list"><div><dt>CI 编码</dt><dd><code>{{ selectedCi.ciCode }}</code></dd></div><div><dt>类型</dt><dd :title="String(selectedCi.ciType)">{{ displayLabel(typeLabels, selectedCi.ciType) }}</dd></div><div><dt>环境</dt><dd :title="String(selectedCi.environment)">{{ displayLabel(environmentLabels, selectedCi.environment) }}</dd></div><div><dt>负责人</dt><dd>{{ selectedCi.ownerName || '未设置' }}</dd></div><div><dt>访问地址</dt><dd><code>{{ displayEndpoint(selectedCi.endpoint) }}</code></dd></div><div><dt>说明</dt><dd>{{ selectedCi.description || '暂无说明' }}</dd></div></dl></aside>
    </section>
    <DetailPanel v-if="inspectorOpen && selectedCi" title="配置项详情" :subtitle="String(selectedCi.ciCode)" @close="inspectorOpen = false"><dl class="oa-definition-list"><div><dt>名称</dt><dd>{{ selectedCi.ciName }}</dd></div><div><dt>类型</dt><dd :title="String(selectedCi.ciType)">{{ displayLabel(typeLabels, selectedCi.ciType) }}</dd></div><div><dt>环境</dt><dd :title="String(selectedCi.environment)">{{ displayLabel(environmentLabels, selectedCi.environment) }}</dd></div><div><dt>状态</dt><dd><StatusBadge :value="String(selectedCi.status)" /></dd></div><div><dt>负责人</dt><dd>{{ selectedCi.ownerName || '未设置' }}</dd></div><div><dt>Endpoint</dt><dd><code>{{ displayEndpoint(selectedCi.endpoint) }}</code></dd></div><div><dt>说明</dt><dd>{{ selectedCi.description || '暂无说明' }}</dd></div></dl></DetailPanel>
    <BaseModal v-if="showCiForm" :title="editingId ? '编辑配置项' : '新增配置项'" @close="showCiForm = false"><form class="form-grid" @submit.prevent="saveCi"><label>CI 编码<input v-model.trim="ciForm.ciCode" required maxlength="64" /></label><label>名称<input v-model.trim="ciForm.ciName" required maxlength="128" /></label><label>类型<select v-model="ciForm.ciType"><option value="SERVICE">服务</option><option value="DATABASE">数据库</option><option value="CACHE">缓存</option><option value="QUEUE">消息队列</option><option value="GATEWAY">网关</option></select></label><label>环境<select v-model="ciForm.environment"><option v-for="(name, value) in environmentLabels" :key="value" :value="value">{{ name }}</option></select></label><label>负责人<input v-model.trim="ciForm.ownerName" maxlength="64" /></label><label>状态<select v-model="ciForm.status"><option value="ACTIVE">正常</option><option value="INACTIVE">停用</option></select></label><label class="full">访问地址<input v-model.trim="ciForm.endpoint" maxlength="255" /></label><label class="full">说明<textarea v-model.trim="ciForm.description" rows="3" maxlength="500" /></label><p v-if="error" class="form-error full">{{ error }}</p><div class="form-actions full"><button type="button" class="button secondary" @click="showCiForm = false">取消</button><button class="button primary" :disabled="saving">{{ saving ? "保存中…" : "保存" }}</button></div></form></BaseModal>
