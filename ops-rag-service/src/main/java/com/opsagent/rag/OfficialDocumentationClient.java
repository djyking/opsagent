 ...
 */
@Component
class OfficialDocumentationClient {
    ...
    static final Map<String, Source> CATALOG =
            Map.of(
                    ...
                    new Source(
                            ...
                            List.of("sentinel", "failover", "quorum", "master")),
                    "MYSQL_CONNECTIONS",
                    new Source(
                            "MySQL 连接管理",
                            "https://dev.mysql.com/doc/refman/8.4/en/too-many-connections.html",
                            List.of("connections", "max_connections", "thread", "client")),
                    "PROMETHEUS_QUERY",
                    new Source(
                            "Prometheus 查询基础",
                            "https://prometheus.io/docs/prometheus/latest/querying/basics/",
                            List.of("query", "metric", "time", "expression")),
                    "DOCKER_HEALTH",
                    new Source(
                            "Docker 健康检查",
                            "https://docs.docker.com/reference/dockerfile/",
                            List.of("healthcheck", "container", "healthy", "interval")),
                    "KUBERNETES_TROUBLESHOOTING",
                    new Source(
                            "Kubernetes Pod 与 Service 排障",
                    ...
                    new Source(
                            ...
                            List.of("query", "search", "retrieval", "vector")));
    private static final Pattern NON_CONTENT =
            Pattern.compile(
                    "(?is)<(head|script|style|nav|aside|header|footer|svg|form)\\b[^>]*>.*?</\\1\\s*>");
    private static final Pattern MAIN_CONTENT =
            Pattern.compile("(?is)<(article|main)\\b[^>]*>(.*?)</\\1\\s*>");
    private final Map<String, Cached> cache = new ConcurrentHashMap<>();
    private final Fetcher fetcher;
    private final Clock clock;

    OfficialDocumentationClient() {
        this(new HttpFetcher(), Clock.systemUTC());
    }

    OfficialDocumentationClient(Fetcher fetcher, Clock clock) {
    ...
    ObjectNode search(String topic) {
        ...
        if (source == null) throw new IllegalArgumentException("只允许已登记的公开技术主题");
        ...
        try {
            ...
            if (!cacheHit) {
                // 缓存键仅限目录主题；失败时删除过期条目，绝不把旧正文说成当前在线证据。
                cache.remove(topic);
    ...
    static List<String> extract(byte[] bytes) throws IOException {
        ...
        var main = MAIN_CONTENT.matcher(html);
        if (main.find()) html = main.group(2);
                ...
                .parse(
                        ...
                        new HTMLEditorKit.ParserCallback() {
                            ...
                            public void handleText(char[] data, int position) {
                                if (line.length() < 4000) line.append(data);
                            }
                            ...
                            public void handleStartTag(
                                ...
                                if (tag == HTML.Tag.PRE) preformatted = true;
                                if (preformatted
                                        && tag == HTML.Tag.SPAN
                                        && (" " + attrs.getAttribute(HTML.Attribute.CLASS) + " ")
                                                .contains(" line ")
                                        && !line.isEmpty()
                            ...
                            public void handleEndTag(HTML.Tag tag, int position) {
                                if (tag.breaksFlow()) flush();
                                if (tag == HTML.Tag.PRE) preformatted = false;
                            }
                            ...
                            public void flush() {
                                String text =
                                        (preformatted
                                                ...
                                                .strip();
                                if (text.length() >= 8 && paragraphs.size() < 1200) {
                                    paragraphs.add(
    ...
    static List<String> rank(List<String> paragraphs, List<String> terms) {
        // Keep short commands with their surrounding explanation instead of discarding them.
        List<String> passages = new ArrayList<>();
        StringBuilder passage = new StringBuilder();
        for (String paragraph : paragraphs) {
            if (!passage.isEmpty() && passage.length() + paragraph.length() + 1 > 1100) {
        ...
        while (selected.size() < 3 && !remaining.isEmpty()) {
            int best = 0;
            int bestNew = -1;
            int bestTotal = -1;
            for (int index = 0; index < remaining.size(); index++) {
                ...
                int fresh =
                        score(
                                candidate,
                                terms.stream().filter(term -> !covered.contains(term)).toList());
                int total = score(candidate, terms);
                if (fresh > bestNew || fresh == bestNew && total > bestTotal) {
                    best = index;
                    bestNew = fresh;
                    bestTotal = total;
                }
            }
            String chosen = remaining.remove(best);
            selected.add(chosen);
            String normalized = chosen.toLowerCase(Locale.ROOT);
            terms.stream().filter(normalized::contains).forEach(covered::add);
        }
