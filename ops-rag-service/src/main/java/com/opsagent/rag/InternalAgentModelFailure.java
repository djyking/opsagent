...
final class InternalAgentModelFailure {
    ...
    static boolean transientFailure(AiProviderException failure) {
        // A timeout or broken connection can hide a completed, billable response. Stop that
        // decision instead of sending the same model request again without a definitive reply.
        return failure.kind() == AiProviderException.FailureKind.HTTP
        ...
        if (code.equals("MODEL_HTTP_401") || code.equals("MODEL_HTTP_403")) {
            ...
            reason =
                    switch (code) {
                        ...
                        case "MODEL_NETWORK_ERROR" -> "AI模型网络连接失败，结果未知且未重复发送；请检查连通性后启动新的隔离演练。";
