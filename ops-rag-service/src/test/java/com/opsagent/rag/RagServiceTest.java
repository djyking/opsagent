...
class RagServiceTest {
    ...
    void shouldExposeIncompleteGenerationAndPreserveItsText() {
        ...
        when(invocation.stream(
                ...
                .thenReturn(
                        new LlmInvocationService.Invocation(
                                new LlmResult(
                                        partial, "deepseek", "model", 20, 4096, false, "length", 2),
                                100));
        RagService service =
                new RagService(
        ...
        RagService.StreamPlan plan =
                new RagService.StreamPlan(
                        "问题",
                        List.of(),
                        List.of(),
                        List.of(),
                        request,
                        new RagService.AnswerMetadata("NONE", false, 0, 0, 0, false, null),
                        null,
                        0);

        RagService.Answer answer =
                service.stream(plan, delta -> {}, new LlmInvocationService.AuditContext(1, "test"));
    ...
    private RerankService rerankService(RagProperties properties, SimpleMeterRegistry metrics) {
